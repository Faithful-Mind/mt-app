<template>
  <section class="m-istyle">
    ...
    <ul class="ibody">
      <li v-for="item in curr" :key="item.title">
...
export default {
  ...
  async mounted () {
    const { status, data: { count, pois } } = await this.$axios.get(
      '/search/resultsByKeywords',
      { params: { keyword: '景点', city: this.$store.state.geo.position.city } }
    )
    if (status === 200 && count > 0) {
      const r = pois.filter(item => item.photos.length).map(item => ({
        title: item.name,
        pos: _.uniq(item.type.split(';')).join('/'),
  ...
  methods: {
    async over (e) {
      ...
      if (dom.tagName.toLowerCase() === 'dd') {
        ...
        if (!this.list[kind].length) {
          const keyword = dom.getAttribute('keyword')
          const { status, data: { count, pois } } = await this.$axios.get(
            '/search/resultsByKeywords',
            { params: { keyword, city: this.$store.state.geo.position.city } }
          )
          if (status === 200 && count > 0) {
            const r = pois.filter(item => item.photos.length).map(item => ({
              title: item.name,
